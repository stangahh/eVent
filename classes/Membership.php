...
class Membership {
    ...
    function get_event_list($id) {
        ...
        if ($response) {
            while ($row = mysqli_fetch_array($response)) {
                array_push($events, "" . $row['event_id'] . "<strong>" . $row['event_name'] . "</strong>, " . $row['event_location'] . "");
            }
        } else {
            echo "FAILURE";
        }
    ...
    function get_event_information($eventid) {
        $event_info = array();        
        $connection = mysqli_connect(DB_SERVER, DB_USER, DB_PASSWORD, DB_NAME) OR die("Database Connection Error: " . mysqli_connect_error());
        $query = "SELECT event_name, event_org_id, event_location, event_latitude, event_longitude, event_postcode, event_amount_funded, event_amount_required, event_creator_user_id, event_date, event_desc, event_photo FROM events WHERE event_id = '" . $eventid . "' LIMIT 1";
        $response = mysqli_query($connection, $query);
    ...
    function get_spec_info($userid) {
        ...
        $connection = mysqli_connect(DB_SERVER, DB_USER, DB_PASSWORD, DB_NAME) OR die("Database Connection Error: " . mysqli_connect_error());
        $query = "SELECT ud_title, ud_fname, ud_lname, ud_username, ud_phone, ud_address, ud_email, ud_dob, ud_sex, ud_occupation FROM user_details WHERE ud_user_id = '" . $userid . "' LIMIT 1";
        $response = mysqli_query($connection, $query);
    ...
    function register_user($un, $pw, $org_id) {
        $connection = mysqli_connect(DB_SERVER, DB_USER, DB_PASSWORD, DB_NAME) OR die("Database Connection Error: " . mysqli_connect_error());        
        $query = "INSERT INTO users (users_username, users_password, users_org_id) VALUES ('" . $un . "','" . MD5($pw) . "','" . $org_id . "')";
        $stmt = mysqli_prepare($connection, $query);        
    ...
    function register_user_details($title, $fn, $ln, $un, $ph, $add, $email, $dob, $sex, $occ) {
        ...
        $query = "INSERT INTO user_details (ud_title, ud_user_id, ud_fname, ud_lname, ud_username, ud_phone, ud_address, ud_email, ud_dob, ud_sex, ud_occupation) VALUES ('" . $title . "', (SELECT users.users_id FROM users WHERE users_username = '" . $un . "'),'" . $fn . "','" . $ln . "','" . $un . "','" . $ph . "','" . $add . "','" . $email . "','" . $dob . "','" . $sex . "','" . $occ . "')";
        $stmt = mysqli_prepare($connection, $query);
        mysqli_stmt_execute($stmt);        
        $affected_rows = mysqli_stmt_affected_rows($stmt);
    ...
    function add_donation($amt, $user, $id) {
        $connection = mysqli_connect(DB_SERVER, DB_USER, DB_PASSWORD, DB_NAME) OR die("Database Connection Error: " . mysqli_connect_error());
        $query = "INSERT INTO donations (don_id, don_user_id, don_amount_donated, don_event_id)	VALUES ('0','" . $user . "','" . $amt . "','" . $id . "')";
        $stmt = mysqli_prepare($connection, $query);      
        mysqli_stmt_execute($stmt);
        $affected_rows = mysqli_stmt_affected_rows($stmt);
    ...
    function set_org($user, $org) {
        $connection = mysqli_connect(DB_SERVER, DB_USER, DB_PASSWORD, DB_NAME) OR die("Database Connection Error: " . mysqli_connect_error());        
        $query = "UPDATE users SET users_org_id = '" . $org . "' WHERE users_id = '" . $user . "'";        
        $stmt = mysqli_prepare($connection, $query);
        mysqli_stmt_execute($stmt);        
        $affected_rows = mysqli_stmt_affected_rows($stmt);
